...
export default function SOFRIORBChart({ observations, latest, updatedAt }: SOFRIORBChartProps) {
  ...
  const points = useMemo(() => {
    ...
    const preset = ranges.find((range) => range.label === activeRange) ?? ranges[3];
  ...
  const chart = useMemo(() => {
    ...
    const rawMin = Math.min(...values, 0, 30);
    const rawMax = Math.max(...values, 0, 30);
    ...
    return { line, x, y, yTicks, xTicks, zeroY: y(0), stressY: y(30) };
  }, [points]);
  ...
  if (!chart || !activePoint) {
    return (
      <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-6 text-neutral-400">
        Run <code className="text-neutral-200">python3 scripts/SOFR-IORB.py</code> to generate chart data.
      </div>
    );
  }
    ...
    <section className="rounded-lg border border-neutral-800 bg-neutral-950 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-400">
              {formatDate(activePoint.date)} {isInspecting && latest !== null ? "· Latest: " + latest.toFixed(1) + " bp" : ""}
              {isInspecting && activePoint.sofr !== null && typeof activePoint.sofr === "number" ? " · SOFR: " + activePoint.sofr.toFixed(2) + "%" : ""}
              {isInspecting && activePoint.iorb !== null && typeof activePoint.iorb === "number" ? " · IORB: " + activePoint.iorb.toFixed(2) + "%" : ""}
            </p>
          </div>
      ...
      <div className="mt-5 overflow-hidden" onMouseLeave={() => setHoverIndex(null)}>
        <svg
          ...
          role="img"
          aria-label="Interactive line chart of the SOFR minus IORB spread"
        ...
        >
          ...
          <line x1={pad.left} x2={width - pad.right} y1={chart.zeroY} y2={chart.zeroY} stroke="#888888" strokeDasharray="7 7" strokeWidth="1.2" />
          <line x1={pad.left} x2={width - pad.right} y1={chart.stressY} y2={chart.stressY} stroke="#ef4444" strokeDasharray="7 7" strokeWidth="1.2" />
          ...
          {isInspecting ? (
            <>
              ...
              <g transform={"translate(" + Math.min(activeX + 16, width - 245) + " " + Math.max(activeY - 66, 18) + ")"}>
                ...
                <text x="12" y="43" fill="#ffffff" fontSize="18" fontWeight="700">{activePoint.value.toFixed(1)} bp</text>
              </g>
            </>
          ) : null}
        </svg>
      ...
      <p className="mt-4 text-xs leading-5 text-neutral-500">
        Source: FRED SOFR and IORB. Dashed red line marks a 30 bp watchline. {updatedAt ? "Updated " + formatDate(updatedAt) + "." : ""}
      </p>
    </section>
  );
}
