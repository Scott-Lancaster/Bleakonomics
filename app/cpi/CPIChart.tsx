...
export default function CPIChart({ observations, recessions, latest, updatedAt }: CPIChartProps) {
  ...
  const points = useMemo(() => {
    ...
    const preset = ranges.find((range) => range.label === activeRange) ?? ranges[5];
  ...
  const chart = useMemo(() => {
    ...
    const rawMin = Math.min(...values, 0, 2);
    const rawMax = Math.max(...values, 0, 2);
    ...
    return { line, x, y, yTicks, xTicks, zeroY: y(0), targetY: y(2), recessionBands };
  }, [points, recessions]);
  ...
  if (!chart || !activePoint) {
    return (
      <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-6 text-neutral-400">
        Run <code className="text-neutral-200">python3 scripts/CPI.py</code> to generate chart data.
      </div>
    );
  }
    ...
    <section className="rounded-lg border border-neutral-800 bg-neutral-950 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-base font-bold text-white">
              CPI Inflation{latestDataParen(maxDate)}
            </p>
            ...
            <p className="mt-1 text-sm text-neutral-400">
              ...
              {isInspecting && activePoint.cpi !== null && typeof activePoint.cpi === "number" ? " · CPI: " + activePoint.cpi.toFixed(3) : ""}
            </p>
          </div>
      ...
      <div className="mt-5 overflow-hidden" onMouseLeave={() => setHoverIndex(null)}>
        <svg
          ...
          role="img"
          aria-label="Interactive line chart of CPI year-over-year inflation"
        ...
        >
          ...
          {chart.recessionBands.map((band, index) => (
            <rect key={index} x={band.x} y={pad.top} width={band.width} height={height - pad.top - pad.bottom} fill="#7f1d1d" opacity="0.28" />
          ))}
          ...
          <line x1={pad.left} x2={width - pad.right} y1={chart.zeroY} y2={chart.zeroY} stroke="#888888" strokeDasharray="7 7" strokeWidth="1.0" />
          <line x1={pad.left} x2={width - pad.right} y1={chart.targetY} y2={chart.targetY} stroke="#ef4444" strokeDasharray="7 7" strokeWidth="1.2" />
      ...
      <p className="mt-4 text-xs leading-5 text-neutral-500">
        Source: FRED CPIAUCSL and USREC. Red bands mark NBER recession periods. Dashed red line marks a 2% reference. {updatedAt ? "Updated " + formatDate(updatedAt) + "." : ""}
      </p>
    </section>
  );
}
