...
export default function OilChart({
  ...
  const chart = useMemo(() => {
    ...
    const zeroY = y(0);
    const showZero = minValue < 0 && maxValue > 0;
    ...
    return { line, x, y, zeroY, showZero, yTicks, xTicks, recessionBands };
  }, [points, recessions]);
  ...
  if (!chart || !activePoint) {
    return (
      <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-6 text-neutral-400">
        Run <code className="text-neutral-200">python3 scripts/Oil.py</code> to generate
        interactive chart data at <code className="text-neutral-200">public/data/oil.json</code>.
      </div>
    );
  }
    ...
    <section className="rounded-lg border border-neutral-800 bg-neutral-950 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-base font-bold text-white">
              WTI Crude Oil{latestDataParen(maxDate)}
            </p>
            <p className="mt-2 text-4xl font-bold text-white">
              {formatDollars(activePoint.value)}
              <span className="ml-2 text-lg font-semibold text-neutral-500">/ barrel</span>
            </p>
            <p className="mt-1 text-sm text-neutral-400">
              {formatDate(activePoint.date)}
              {isInspecting && latest !== null ? " · Latest: " + formatDollars(latest) : ""}
            </p>
          </div>
      ...
      <div className="mt-5 overflow-hidden" onMouseLeave={() => setHoverIndex(null)}>
        <svg
          ...
          role="img"
          aria-label="Interactive line chart of the WTI crude oil spot price"
          ...
          {isInspecting ? (
            <>
              ...
              <g transform={"translate(" + Math.min(activeX + 16, width - 245) + " " + Math.max(activeY - 66, 18) + ")"}>
                ...
                <text x="12" y="43" fill="#ffffff" fontSize="18" fontWeight="700">{formatDollars(activePoint.value)}</text>
              </g>
            </>
          ) : null}
        </svg>
