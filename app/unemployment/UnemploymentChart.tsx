...
export default function UnemploymentChart({
  ...
  const chart = useMemo(() => {
    ...
    const padding = Math.max((rawMax - rawMin) * 0.12, 0.35);
  ...
  if (!chart || !activePoint) {
    return (
      <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-6 text-neutral-400">
        Run <code className="text-neutral-200">python3 scripts/Unemployment.py</code> to generate chart data.
      </div>
    );
  }
    ...
    <section className="rounded-lg border border-neutral-800 bg-neutral-950 p-4 sm:p-6">
      ...
      <div className="mt-5 overflow-hidden" onMouseLeave={() => setHoverIndex(null)}>
        <svg
          ...
          role="img"
          aria-label="Interactive line chart of the U.S. unemployment rate"
      ...
      <p className="mt-4 text-xs leading-5 text-neutral-500">
        Source: FRED UNRATE and USREC. Red bands mark NBER recession periods. {updatedAt ? "Updated " + formatDate(updatedAt) + "." : ""}
      </p>
    </section>
  );
}
