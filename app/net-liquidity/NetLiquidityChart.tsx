  ...
  if (!chart || !activePoint) {
    return (
      <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-6 text-neutral-400">
        Run <code className="text-neutral-200">python3 scripts/NetLiquidity.py</code> to generate
        interactive chart data at <code className="text-neutral-200">public/data/net_liquidity.json</code>.
      </div>
    );
  }
    ...
    <section className="rounded-lg border border-neutral-800 bg-neutral-950 p-4 sm:p-6">
      ...
      <div className="mt-5 overflow-hidden" onMouseLeave={() => setHoverIndex(null)}>
        <svg
          ...
          role="img"
          aria-label="Interactive line chart of US net liquidity"
          ...
          {isInspecting ? (
            <>
              ...
              <g transform={"translate(" + Math.min(activeX + 16, width - 245) + " " + Math.max(activeY - 66, 18) + ")"}>
                ...
                <text x="12" y="43" fill="#ffffff" fontSize="18" fontWeight="700">{formatTrillions(activePoint.value)}</text>
              </g>
            </>
          ) : null}
        </svg>
      ...
      <p className="mt-4 text-xs leading-5 text-neutral-500">
        Source: FRED WALCL − WTREGEN − RRPONTSYD. Weekly from 2002. Red bands mark NBER recession periods. {updatedAt ? "Updated " + formatDate(updatedAt) + "." : ""}
      </p>
    </section>
  );
}
